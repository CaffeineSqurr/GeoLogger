...
function select_box(req, res, next){
  ...
  var query = req.query;
  var limit = (typeof(query.limit) !== "undefined") ? query.limit : 400;
...
function add_point(req, res, next){
  var point = req.query;
  if(!(Number(point.lat)
    && Number(point.lon)))
  {
    res.send(500, {http_status:400,error_msg: "this endpoint requires a pair of lat, long coordinates: lat lon\n"});
    return console.error('could not connect to postgres', err);
  }



  var pin = {"pos" : [point.lon, point.lat], "Name" : point.name};
  var insert = "Insert into "+table_name+" (name, the_geom) VALUES ";
  var qpoints = insertMapPinSQL(pin);
  var query = insert + qpoints + ';';
  console.log(query);
  pg(query, function(err) {
    if(err) {
      return console.error(error_response, err);
    }
     res.send(200);
